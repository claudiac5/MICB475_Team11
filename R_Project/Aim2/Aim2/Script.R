# =========================
# Aim 2: Subtype-Associated Differential Taxa
# =========================
...
keep_taxa_mc <- !(
  grepl("mitochondria", tax_df_ps2$Class, ignore.case = TRUE) |
    grepl("mitochondria", tax_df_ps2$Order, ignore.case = TRUE) |
    grepl("mitochondria", tax_df_ps2$Family, ignore.case = TRUE) |
    grepl("chloroplast", tax_df_ps2$Class, ignore.case = TRUE) |
    grepl("chloroplast", tax_df_ps2$Order, ignore.case = TRUE) |
    grepl("chloroplast", tax_df_ps2$Family, ignore.case = TRUE)
)
...
# -------------------------
# choose significant genera for the heatmap
# if ANCOM-BC2 yields too few, expand with indicator taxa
# -------------------------
sig_genera <- unique(ancom_sig$Genus)

if (length(sig_genera) < 5) {
  sig_genera <- unique(c(ancom_sig$Genus, ind_sig$Genus))
}

# -------------------------
# build a sample-by-genus matrix for heatmap plotting
# -------------------------
...
# -------------------------
# log-transform the heatmap matrix with a small pseudocount
# -------------------------
heat_mat_log <- log10(heat_mat + 1e-6)

# -------------------------
# create the subtype annotation data frame for the heatmap
# -------------------------
...
# -------------------------
# save the heatmap of significant genera
# -------------------------
png("../figures/aim2_significant_genera_heatmap.png", width = 1800, height = 1400, res = 200)
pheatmap(
  mat = heat_mat_log,
